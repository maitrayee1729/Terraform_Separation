...
icinga2_master="icinga2-in.unicommerce.infra"
icinga2_master_port=5665
plugins_path=/usr/lib64/nagios/plugins
notification=${1:-icingaadmins}
instance_id=`curl -s http://169.254.169.254/latest/meta-data/instance-id`
ticket=`curl -k -s -u icingaweb2:Wijsn8Z9eRs5E25d -H 'Accept: application/json' -X POST 'https://'$icinga2_master':5665/v1/actions/generate-ticket' -d '{ "cn": "'$hostname'" }' | grep -Eo "\"ticket\":\"(.+?)\"" | cut -d: -f2 | cut -d\" -f2`
...
rm -f /var/lib/icinga2/api/zones/global-templates/_etc/command.conf
